Gis.Show = $.extend({}, {
    ...
    queryServerCallBack:function(response){
        for(var i=0;i<response.features.length;i++){
            ...
            Gis.Map.showLayer2.add(feature);
        }
    },
    queryPointCallBack:function(response,type){
        ...
        for(var i=0;i<response.length;i++){
            ...
            Gis.Map.showLayer3.add(graphic);
    ...
    queryPolygonCallBack:function(response,type){
        var symbol = Gis.Symbol.gxd[type];
        for(var i=0;i<response.length;i++){
            var geom = Gis.Util.JsonToGeometry(response[i].GEOMSTRY);
            if(geom==null){
            ...
            if(type=="loop"){
                ...
                point = new esri.geometry.Point(extent.xmin+((extent.xmax-extent.xmin)/6),
                    extent.ymin-((extent.ymin-extent.ymax)/8), Gis.Map.gxdMap.spatialReference);
            }else{
                point = geom.getExtent().getCenter();
            }
            var text=(isNull(response[i].LABEL)!=""?response[i].LABEL:response[i].NAME);
            Gis.Map.textLayer.add(new esri.Graphic(point,Gis.Symbol.getTextSymbol(text),null));
        }
